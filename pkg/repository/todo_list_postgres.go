...
func (r *TodoListPostgres) GetAll(userId int) ([]todo_app.Todolist, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.tittle, tl.description FROM %s tl INNER JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1", todoListsTable, usersListsTable)
...
func (r *TodoListPostgres) GetById(userId, listId int) (todo_app.Todolist, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.tittle, tl.description FROM %s tl INNER JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1 AND ul.list_id = $2", todoListsTable, usersListsTable)
...
func (r *TodoListPostgres) Update(userId, listId int, input todo_app.UpdateListInput) error {
	...
	if input.Title != nil {
		setValues = append(setValues, fmt.Sprintf("tittle = $%d", argId))
